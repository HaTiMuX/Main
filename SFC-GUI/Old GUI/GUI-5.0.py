...
class MainFrame(QtGui.QFrame):
	def __init__(self, parent):
		...
		self.updateSF = QtGui.QPushButton("&Update SF Function",self)
...
class updateFunction_Frame(QtGui.QFrame):
	def __init__(self, parent):
		...
		self.addLocator.setToolTip('Add new Locator')
		self.updateLocator = QtGui.QPushButton("&Update Locator",self)
		self.updateLocator.setToolTip('Update existing Locators')
		self.delLocator = QtGui.QPushButton("&Delete Locator",self)
		self.delLocator.setToolTip('Delete existing locators')
       		self.back = QtGui.QPushButton("Back")
...
class addLocator_Frame(QtGui.QFrame):
	def __init__(self, parent):
		...
		#Combo Box
		self.combo = QtGui.QComboBox(self)
		sql = "SELECT SF, Locator1, Locator2, LocNum FROM Locators Where LocNum <= 2"
		try:	
			cursor.execute(sql)
   			results = cursor.fetchall()
		except:
...
class delLocator_Frame(QtGui.QFrame):
	def __init__(self, parent):
		...
		self.success_msg = QtGui.QLabel("")
		self.success_msg.setStyleSheet('color: green')
		choose = QtGui.QLabel("Choose the Entry to update:", self)
		select = QtGui.QLabel("Select the locators to remove:")
		self.checkBoxList.append(QtGui.QCheckBox("Locator 1")) 
		self.checkBoxList.append(QtGui.QCheckBox("Locator 2")) 
		self.checkBoxList.append(QtGui.QCheckBox("Locator 3")) 
		...
		#Main Box
	    	self.vbox = QtGui.QVBoxLayout()
		self.vbox.addWidget(choose)
		self.vbox.addWidget(self.combo)
		self.vbox.addWidget(select)
		self.vbox.addWidget(self.checkBoxList[0])
		self.vbox.addWidget(self.checkBoxList[1])
		self.vbox.addWidget(self.checkBoxList[2])
		self.vbox.addWidget(self.error_msg)
		self.vbox.addWidget(self.success_msg)
		self.vbox.addStretch()
		self.vbox.addLayout(hbox)
...
class MainWindow(QtGui.QMainWindow):
		...
		self.addLocFrame.combo.currentIndexChanged.connect(self.locAdd_comboEventHandler)
		...
		self.updateLocFrame.combo.currentIndexChanged.connect(self.locUpdate_comboEventHandler)
		#Delete Locator Frame
		self.delLocFrame.back.clicked.connect(self.backToSFUpdate_buttonClicked)
		for checkbox in self.delLocFrame.checkBoxList:
			checkbox.clicked.connect(self.delLoc_checkBoxClicked) 
	...
	#*SF Update FEvents*#
	#*******************#
	def locAdd_comboEventHandler(self):
	...
	def locUpdate_comboEventHandler(self):
		#Reading current SF functions
	...
	def addLoc_buttonClicked(self): #Add locator to an existing SF Function
		if self.addLocFrame.combo.count() != 0:
			...
			if error is None:
				...
				if reply == QtGui.QMessageBox.Yes:
					...
					#Updating List of SF Functions and locators
					if locNum==3:
						self.addLocFrame.combo.removeItem(cb_index)
						self.addLocFrame.loc1.pop(cb_index)
						self.addLocFrame.loc2.pop(cb_index)
	...
	def updateLoc_buttonClicked(self): #updating locators of an existing SF Function
		if self.updateLocFrame.combo.count() != 0:
			...
			if error is None:
				...
				if reply == QtGui.QMessageBox.Yes:
					...
					#Updating List of SF Functions and locators
					if locNum==3:
						self.updateLocFrame.combo.setItemText(cb_index, updatedSF + " => " + self.updateFuncFrame.loc1[cb_index] + "|" + self.updateFuncFrame.loc2[cb_index] + "|" + self.updateFuncFrame.loc3[cb_index])
					elif locNum==2:	
						self.updateLocFrame.combo.setItemText(cb_index, updatedSF + " => " + self.updateFuncFrame.loc1[cb_index] + "|" + self.updateFuncFrame.loc2[cb_index])
					else:
						self.updateLocFrame.combo.setItemText(cb_index, updatedSF + " => " + self.updateFuncFrame.loc1[cb_index])

			else:
				self.updateLocFrame.msg.setText(error)
		else:
			QtGui.QMessageBox.critical(self, 'Error', "No Entry remaining!" , QtGui.QMessageBox.Ok)


	'''def delLoc_checkBoxClicked(self):
		global locCount
        	sender = self.sender()
		loc = sender.text()
		...
		if sender.isChecked() is True:
			if(locCount<=2):
				if loc=="Locator 1":
					self.delLocFrame.loc1 = True
	
				elif loc=="Locator 2":
					self.delLocFrame.loc2 = True

				else:
					self.delLocFrame.loc3 = True 
			else:
