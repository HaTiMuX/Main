...
def Update_Del(Index, rowMap):
	...
	try:
		...
		print "Error: connecting to remote server %s failed! (SFCRoutingTable delete update failed)" % IP
...
class MainWindow(QtGui.QMainWindow):
					...
					if IPCond1 is False:
						...
						if reply == QtGui.QMessageBox.Yes:
							try:
								...
								sql = "INSERT INTO Locators (SF, Locator1, Locator2, Locator3) VALUES ('%s', '%s', '%s', '%s')" % (sf, locator1, locator2, locator3)
								cursor.execute(sql)
							   	db.commit()
					
								#Updating List of SF Functions and locators
								self.updateFuncFrame.combo.addItem(sf + " => " + locator1 + "|" + locator2 + "|" + locator3)	
								self.delFuncFrame.combo.addItem(sf + " => " + locator1 + "|" + locator2 + "|" + locator3)	
	...
	def updateFunc_buttonClicked(self): #updating the locator of an existing SF Function
		if self.delFuncFrame.combo.count() != 0:
			...
			SF = curtext.split(' ')
			locator1 = self.updateFuncFrame.newLoc1_le.text()
			locator2 = self.updateFuncFrame.newLoc2_le.text()
			locator3 = self.updateFuncFrame.newLoc3_le.text()

			IPExp = "^((25[0-5]|2[0-4][0-9]|1?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|1?[0-9][0-9]?)$"
			emptyExp = "^( ?){15}$"


			cond = (re.search(emptyExp, locator1) is not None) and (re.search(emptyExp, locator2) is not None) and (re.search(emptyExp, locator3) is not None)

			cond1 = (re.search(IPExp, locator1) is None) and (re.search(emptyExp, locator1) is None)
			cond2 = (re.search(IPExp, locator2) is None) and (re.search(emptyExp, locator2) is None)
			cond3 = (re.search(IPExp, locator3) is None) and (re.search(emptyExp, locator3) is None)
			
			if cond:
				self.updateFuncFrame.msg.setText("Type at least 1 locator to update!")

			elif cond1 or cond2 or cond3:
				self.updateFuncFrame.msg.setText("Type valid IP addresses!")
				...
				if reply == QtGui.QMessageBox.Yes:
					try:
						if re.search(emptyExp, locator1) is None:
							sql1 = "UPDATE Locators SET Locator1 = '%s' WHERE SF='%s'" % (locator1, SF[0])
							cursor.execute(sql1)

						if re.search(emptyExp, locator2) is None:
							sql2 = "UPDATE Locators SET Locator2 = '%s' WHERE SF='%s'" % (locator2, SF[0])
							cursor.execute(sql2)

						if re.search(emptyExp, locator3) is None:
							sql3 = "UPDATE Locators SET Locator3 = '%s' WHERE SF='%s'" % (locator3, SF[0])
							cursor.execute(sql3)
		  				...
		  				db.rollback()
						print "Error Updating SF Locator"

					index = self.delFuncFrame.combo.currentIndex()
					self.updateFuncFrame.combo.setItemText(index, SF[0] + " => " + locator1 + "|" + locator2 + "|" + locator3)	
					self.delFuncFrame.combo.setItemText(index, SF[0] + " => " + locator1 + "|" + locator2 + "|" + locator3)	
		else:
			QtGui.QMessageBox.critical(self, 'Error', "No Entry remaining!" , QtGui.QMessageBox.Ok)
